...
from ml.sentence_transformers.tokenizer.PhoTokenizer import PhoTokenizer
import json
...
class PhoBERT(nn.Module):
    ...
    """
    def __init__(self, model_path: str, max_seq_length: int = 128, do_lower_case: Optional[bool] = False,
                 model_args=None, tokenizer_args=None):
        ...
        if max_seq_length > 256:
            logging.warning("PhoBERT only allows a max_seq_length of 256 (258 with special tokens). "
                            "Value will be set to 256")
            max_seq_length = 256
