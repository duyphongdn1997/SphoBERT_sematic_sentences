...
class PhoTokenizer(object):
    ...
    def cls_token_id(self):
        """ Id of the classification token in the vocabulary. E.g. to extract a summary of an input sequence
        leveraging self-attention along the full depth of the model. """
        return self.vocab.bos_index

    def __init__(self, bpe_path: str, vncorenlp_path: str = None, do_lower_case: bool = False):
        ...
        self.pho_config = BPEConfig(vncorenlp=os.path.join(self.vn_core_path, "VnCoreNLP/VnCoreNLP-1.1.1.jar"),
                                    bpe_codes=bpe_codes_path, vocab=vocab_file_path)
    ...
    def save(self, output_path: str):
        with open(os.path.join(output_path, CONFIG_FILE), 'w') as fOut:
            json.dump(
                {'bpe_path': self.pho_config.vocab.replace(VOCAB_FILE, ''), 'vncorenlp_path': self.pho_config.vncorenlp,
    ...
    def prepare_for_model(
        ...
        Args:
            ...
            pad_to_max_length: if set to True, the returned sequences will be padded according to the model'spadding
            side and padding index, up to their max length. If no max length is specified, the padding is done up to
            ...
            return_token_type_ids: (optional) Set to False to avoid returning token_type_ids
            (default: set to model specifics).
            return_attention_mask: (optional) Set to False to avoid returning attention mask
            (default: set to model specifics)
            return_overflowing_tokens: (optional) Set to True to return overflowing token information
            (default False).
            return_special_tokens_mask: (optional) Set to True to return special tokens mask information
            (default False).
            ...
            A Dictionary of shape::
                {
                    ...
                    special_tokens_mask: list[int] if ``add_special_tokens`` if set to ``True`` and
                    return_special_tokens_mask is True
        ...
        needs_to_be_padded = pad_to_max_length and (
                ...
                and len(encoded_inputs["input_ids"]) < self.model_max_length <= LARGE_INTEGER
        )
    ...
    def truncate_sequences(
        ...
        if truncation_strategy == "longest_first":
            ...
            raise ValueError(
                "Truncation_strategy should be selected in ['longest_first', 'only_first', 'only_second', "
                "'do_not_truncate'] "
            )
    ...
    def batch_encode_plus(
            ...
            A Dictionary of shape::
                {
                    ...
                    overflowing_tokens: list[List[int]] if a ``max_length`` is specified and return_overflowing_tokens
                    is True
                    num_truncated_tokens: List[int] if a ``max_length`` is specified and return_overflowing_tokens
                    is True
                    special_tokens_mask: list[List[int]] if ``add_special_tokens`` if set to ``True`` and
                    return_special_tokens_mask is True
                }
            With the fields:
            - ``input_ids``: list of token ids to be fed to a model
        ...
        if pad_to_max_length and self.pad_token_id is None:
            raise ValueError(
                "Unable to set proper padding strategy as the tokenizer does not have a padding token. In this case "
                "please set the `pad_token` `(tokenizer.pad_token = tokenizer.eos_token e.g.)` or add a new pad token "
                "via the function add_special_tokens if you want to use a padding strategy "
            )
